...
class Application(tk.Frame):
    ...
    def addarea(self):
        try:
            ...
            messagebox.showinfo("ERROR","输入错误")
            return
        acb=ACB(size)
        if len(self.area)==0:
            if acb.star+acb.size>62:
                messagebox.showinfo("ERROR","分区大小过大")
    ...
    def showarea(self):
        ...
        try:
            ...
            messagebox.showinfo("ERROR","分区插入表格出错")

    def addjcb(self):
        s=self.jobentry.get().split()
        if len(s)!=2:
            messagebox.showinfo("ERROR","输入有误")
            return
        try:
            s[1]=int(s[1])
        except:
            messagebox.showinfo("ERROR","作业大小有误")
            return
        if int(s[1])>62:
            messagebox.showinfo("ERROR","不能接受的作业")
            return
        job=JCB(s[0],int(s[1]))
        jf=0
        for i in self.jcb0:
            if s[0]==i.name:
                messagebox.showinfo("ERROR","作业名重复")
                jf=1
        if jf==0:
    ...
    def showjcb(self):
        ...
        try:
            ...
            messagebox.showinfo("ERROR","作业插入表格出错")

    def distribut(self):#使用类似最佳适应分配,把作业队列中的第一个分配到内存
        if len(self.jcb0)==0:
            messagebox.showinfo("ERROR","当前没有要分配的作业")
            return
        cr=self.jcb0[0]
        smin=62
        sa=-1
        ...
        if sa!=-1:
            ...
            messagebox.showinfo("ERROR","目前还不能分配")

    def cancle(self):
        if len(self.jcb1)==0:
            messagebox.showinfo("ERROR","当前没有可撤销的作业")
            return
        cr=self.jcb1[0]
        for i in self.area:
