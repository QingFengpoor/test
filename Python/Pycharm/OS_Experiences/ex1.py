...
class Application(tk.Frame):
    ...
    def showPCB(self,LPCB):
        ...
        treeview.grid(row=3,column=0,columnspan=2,rowspan=len(LPCB)+1)
        try:
            for i in range(len(LPCB)):
                treeview.insert('',i,values=(LPCB[i].name,LPCB[i].ctime,LPCB[i].ntime))
        except:
            messagebox.showinfo("ERROR","Show error")
